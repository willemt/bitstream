...
)
{
    ...
    unsigned char *cptr_write = &bits[0];
    unsigned char *cptr_read = &bits[0];
    bitstream_init(bits, 1000);
...
)
{
    ...
    CuAssertTrue(tc, 0x0FF == bitstream_read_ubyte(&cptr_read));
...
)
{
    ...
    cptr_read = &num[0];
    CuAssertTrue(tc, 0xF0 == bitstream_read_ubyte(&cptr_read));
